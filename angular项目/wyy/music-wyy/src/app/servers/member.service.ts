...
import {RecordVal, Singin, User, UserRecord, UserSheet} from './data-types/member.type';
...
export class MemberService {
  ...
  // 签到
  signin(): Observable<Singin> {
    const params = new HttpParams({ fromString: queryString.stringify({ type: 1 }) });
    return this.http.get(this.uri + 'daily_signin', { params })
      .pipe(map(res => res as Singin));
  }

  // 听歌记录
  getUerRecord(uid: string, type = RecordType.weekData): Observable<RecordVal[]> {
  ...
  // 用户歌单
  getUerSheets(uid: string): Observable<UserSheet> {
    const params = new HttpParams({ fromString: queryString.stringify({ uid }) });
